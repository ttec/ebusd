  ...
  if (message == nullptr) {
    ...
    if (result < RESULT_OK) {
      ...
      if (m_answer && dstAddress == (master ? m_ownMasterAddress : m_ownSlaveAddress)) {
        logNotice(lf_update, "%s %s self-update %s %s QQ=%2.2x: %s | %s / %s", prefix, mode, circuit.c_str(), name.c_str(),
        ...
        if (message->getSrcAddress() == SYN) {  // any destination and any source
          logNotice(lf_update, "%s %s %s %s QQ=%2.2x ZZ=%2.2x: %s | %s / %s", prefix, mode, circuit.c_str(), name.c_str(),
              srcAddress, dstAddress, data.c_str(), m_command.getStr().c_str(), m_response.getStr().c_str());
        } else {
          logNotice(lf_update, "%s %s %s %s ZZ=%2.2x: %s | %s / %s", prefix, mode, circuit.c_str(), name.c_str(), dstAddress,
              data.c_str(), m_command.getStr().c_str(), m_response.getStr().c_str());
        }
      } else if (message->getSrcAddress() == SYN) {  // any source
        logNotice(lf_update, "%s %s %s %s QQ=%2.2x: %s | %s / %s", prefix, mode, circuit.c_str(), name.c_str(), srcAddress,
            data.c_str(), m_command.getStr().c_str(), m_response.getStr().c_str());
      } else {
        logNotice(lf_update, "%s %s %s %s: %s | %s / %s", prefix, mode, circuit.c_str(), name.c_str(), data.c_str(),
            m_command.getStr().c_str(), m_response.getStr().c_str());
      }
    }
  }
}
